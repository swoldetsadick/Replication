...
<table style="width:50%" border="9" align="center"> <caption>Differences Between R & STATA Generated csur92 Data Set</caption> <tr> <td>identif</td><td>anno</td><td>R</td><td>STATA</td></tr> <tr> <td>11636</td><td>1991</td><td>1</td><td>0</td></tr></table><br>
...
<table style="width:50%" border="9" align="center"> <caption>Differences Between R & STATA Generated csur95 Data Set</caption> <tr> <td>identif</td><td>anno</td><td>R</td><td>STATA</td></tr> <tr> <td>1839</td><td>1992</td><td>1</td><td>0</td></tr></table><br>
...
```{r, cache = TRUE}
csur98 <- subset(data,ind98 == 1 & dip97 != "." & dip97 > 0 & eta98 >= 0 )
#Size as Proxied by Total Sales;
csur98$lsize <- log(csur98$sales)
```
...
#Dummies M&A's;
csur98$duacq <- ifelse(csur98$acqui==1,1,ifelse(is.na(csur98$scorpo),0,0))
csur98$dusco<- ifelse(csur98$scorpo==1,1,ifelse(is.na(csur98$scorpo),0,0))

#Dummy Industrial Group
csur98$durg=ifelse(csur98$isgru==1,1,0)
...
csur98$roa <- csur98$util/(csur98$aco+csur98$aim)
	#Financial Expenses/Operating Profits
csur98$ofut <- csur98$of/(csur98$of+csur98$utiln)
	#Share of Tangible Assets over Total Assets
csur98$quoimm <- csur98$imte/(csur98$aco+csur98$aim)
	#Log of Sales
csur98$lsales <- log(csur98$sales)
...
csur98$duofut <- ifelse(csur98$ofut > csur98$pofut & csur98$ofut != ".",1,ifelse(csur98$ofut==".",".",0))
csur98$dimm <- ifelse(csur98$quoimm < csur98$pquoimm & csur98$pquoimm != ".",1,ifelse(csur98$ofut==".",".",0))
```
...
csur98$desi <- ifelse(csur98$desi==1,1,ifelse(csur98$desi==2,0,ifelse(csur98$desi=="." & csur98$paga=="." & csur98$chie==".",NA,0)))
csur98$desi98 <- csur98$desi

csur98[grep("acqui",colnames(csur98))] <- NULL
csur98[grep("scorpo",colnames(csur98))] <- NULL
csur98[grep("isgru",colnames(csur98))] <- NULL
...
```{r, cache = TRUE}
csur01 <- subset(data, ind01 == 1 & dip00 != "."  &  dip00 > 0 & eta01 >= 0)

#Size as Proxied by Total Sales;
csur01$lsize <- log(csur01$sales)

#Dummy Rationing Baseline def.;
csur01$razd <- ifelse(csur01$desi==1 & csur01$paga==1,1,ifelse(csur01$desi=="." & csur01$paga=="."& csur01$chie==".",NA,0))
csur01$raz98d <- csur01$razd

#Dummies M&A's;
csur01$duacq <- ifelse(csur01$acqui==1,1,0)
csur01$dusco<- ifelse(csur01$scorpo==1,1,0)

#Dummy Industrial Group
csur01$durg=ifelse(csur01$isgru==1,1,0)
...
csur01$roa <- csur01$util/(csur01$aco+csur01$aim)
        #Financial Expenses/Operating Profits
csur01$ofut <- csur01$of/(csur01$of+csur01$utiln)
	#Share of Tangible Assets over Total Assets
csur01$quoimm <- csur01$imte/(csur01$aco+csur01$aim)
	#Log of Sales
csur01$lsales <- ifelse(log(csur01$sales)==NA|is.infinite(log(csur01$sales))==T, NA, log(csur01$sales))
...
csur01$duofut <- ifelse(csur01$ofut > csur01$pofut & csur01$ofut != ".",1,ifelse(csur01$ofut==".",".",0))
csur01$dimm <- ifelse(csur01$quoimm < csur01$pquoimm & csur01$pquoimm != ".",1,ifelse(csur01$ofut==".",".",0))

#Dummy Desiring more Credit

csur01$desi <- ifelse(csur01$desi==1,1,ifelse(csur01$desi==2,0,ifelse(csur01$desi=="." & csur01$paga=="." & csur01$chie==".",NA,0)))
csur01$desi98 <- csur01$desi

csur01[grep("acqui",colnames(csur01))] <- NULL
csur01[grep("scorpo",colnames(csur01))] <- NULL
csur01[grep("isgru",colnames(csur01))] <- NULL
...
```{r, cache = TRUE}
suppressMessages(library(lubridate))
csuro95 <- subset(csur95, year(csur95$anno) == 1994)
...
What is done in the code chunk below is to compute the missing age data of a firm present in both surveys, but whose age in one survey is missing. Hence, a firm present in both 95 and 92 survey whose age in 95 is there, but for whom age in 92 is missing or illogical, age in 92 is set equal to age in 95 - 3.<br>
...
dim(csur92)
names(csur92)
summary(round(csur92$lsize, digits=3))
summary(as.factor(csur92$razd))
summary(as.factor(csur92$raz92d))
summary(as.factor(csur92$duacq))
summary(as.factor(csur92$dusco))
summary(csur92$dupav1)
summary(csur92$dupav2)
summary(csur92$dupav3)
summary(csur92$dupav4)
summary(csur92$roa)
summary(csur92$ofut)
summary(csur92$quoimm)
summary(csur92$lsales)
summary(csur92$pofut)
summary(csur92$pquoimm)
summary(as.factor(csur92$duofut))
summary(as.factor(csur92$dimm))
